...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/manage-moderators.css">
...
<body>
	...
	<!-- Overlay modal for add moderator form.
         Active class is added when showModal attribute is true from servlet -->
	<div id="addModModal"
		class="modal-overlay ${showModal == true ? 'active' : ''}">
		<div class="modal-box">
			<div class="modal-header">
				<h3>Add Moderator</h3>
				<a href="${pageContext.request.contextPath}/ManageModerator"
					class="modal-close"> <svg xmlns="http://www.w3.org/2000/svg"
						width="20" height="20" viewBox="0 0 24 24">
                        <path fill="none" stroke="#64748B"
							stroke-linecap="round" stroke-width="2" d="M18 6L6 18M6 6l12 12" />
                    </svg>
				</a>
			</div>
			<div class="modal-body">
				<c:if test="${showModal == true}">
					<jsp:include
						page="/WEB-INF/protected_pages/views_forInclude/admin-form.jsp" />
				</c:if>
			</div>
		</div>
	</div>
	...
	<article class="main-footer">
		<main>
			<section class="admin-main-outer">
				<section class="admin-dashboar-nav">
					<span class="admin-dashboar-overview">
					...
					</span> <span class="no-admin-dashboar-overview"> <span
						class="admin-dashboard-nav-txt">
						...
						class="admin-dashboard-admin-logo"> <svg
				...
				<c:choose>
					<c:when
						test="${fn:contains(sessionScope.msg, 'Failed to create moderator')}">
						<span class="error-msg">${sessionScope.msg}</span>
					</c:when>
					<c:when
						test="${fn:contains(sessionScope.msg, 'Successfully created new moderator')}">
						<span class="suc-msg">${sessionScope.msg}</span>
					</c:when>
				</c:choose>
				...
				<div class="mm-hero">
					<span class="mm-upper">
						<div class="mm-breadcrumb">
							<span class="mm-breadcrumb-sep"></span> <span class="active">MODERATORS</span>
						</div>
						<div class="mm-hero-left">
							<h1>Manage Moderators</h1>
							<p>Grant and revoke permissions, oversee activity, and manage
								the elite response team for ClaimIt digital concierge.</p>
						</div>
					</span>

					<div class="mm-total-card">
						<p class="mm-total-label">TOTAL MODERATORS</p>
						<p class="mm-total-value">${fn:length(admins)}</p>
						<form action="${pageContext.request.contextPath}/ManageModerator"
							method="POST" class="mm-add-btn">
							<input type="hidden" name="action" value="addMod">
							<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
								viewBox="0 0 24 24">
                                <rect width="24" height="24" fill="none" />
                                <g fill="none" stroke="#005BBF"
									stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
                                    <g stroke-dasharray="22">
                                        <path
									d="M3 21v-1c0 -2.21 1.79 -4 4 -4h4c2.21 0 4 1.79 4 4v1">
                                            <animate fill="freeze"
									attributeName="stroke-dashoffset" dur="0.3s" values="22;0" />
                                        </path>
                                        <path stroke-dashoffset="22"
									d="M9 13c-1.66 0 -3 -1.34 -3 -3c0 -1.66 1.34 -3 3 -3c1.66 0 3 1.34 3 3c0 1.66 -1.34 3 -3 3Z">
                                            <animate fill="freeze"
									attributeName="stroke-dashoffset" begin="0.3s" dur="0.3s"
									to="0" />
                                        </path>
                                    </g>
                                    <g stroke-dasharray="8"
									stroke-dashoffset="8">
                                        <path d="M15 6h6">
                                            <animate fill="freeze"
									attributeName="stroke-dashoffset" begin="0.7s" dur="0.2s"
									to="0" />
                                        </path>
                                        <path d="M18 3v6">
                                            <animate fill="freeze"
									attributeName="stroke-dashoffset" begin="0.9s" dur="0.2s"
									to="0" />
                                        </path>
                                    </g>
                                </g>
							...
							<button type="submit"
								style="display: flex; align-items: center; gap: 8px; padding: 10px 18px; background: #1D4ED8; color: #fff; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; letter-spacing: 1px;">
				...
				<div class="mm-table-section">
					<div class="mm-table-header">
						...
						<div class="mm-table-actions">
							<form action="${pageContext.request.contextPath}/ManageModerator"
								method="GET" class="mm-icon-action" title="Filter">
						...
						<table class="mm-table">
							...
							<tbody>
								<c:forEach var="eachAdmin" items="${admins}">
									<tr>
										<td><span class="mm-mod-id">#${eachAdmin.adminId}</span></td>
										<td>
											<div class="mm-mod-info">
												<div class="mm-avatar"
													style="background: #DBEAFE; color: #1E40AF;">
													<c:set var="nameParts"
														value="${fn:split(eachAdmin.fullName, ' ')}" />
													<c:choose>
														<c:when test="${fn:length(nameParts) >= 2}">
                                                            ${fn:substring(nameParts[0], 0, 1)}${fn:substring(nameParts[1], 0, 1)}
                                                        </c:when>
														<c:otherwise>
                                                            ${fn:substring(eachAdmin.fullName, 0, 1)}
                                                        </c:otherwise>
													</c:choose>
												</div>
												<span class="mm-mod-name">${eachAdmin.fullName}</span>
											</div>
										</td>
										<td><span class="mm-mod-email">${eachAdmin.email}</span></td>
										<td><span class="mm-date"> <fmt:formatDate
													value="${eachAdmin.createdAt}" pattern="MMM dd, yyyy" />
										</span></td>
										<td>
											<div class="mm-actions-cell">
												<c:choose>
													<c:when
														test="${eachAdmin.status == 'ACTIVE' || eachAdmin.status == 'INACTIVE'}">
														<form
															action="${pageContext.request.contextPath}/ManageModerator"
															method="POST">
															<input type="hidden" name="adminId"
																value="${eachAdmin.adminId}"> <input
																type="hidden" name="actionDb" value="SUSPENDED">
															<button type="submit" class="mm-suspend-btn">
																<svg xmlns="http://www.w3.org/2000/svg" width="13"
																	height="13" viewBox="0 0 24 24">
                                                                    <path
																		fill="#854D0E"
																		d="M12 2a10 10 0 1 0 0 20A10 10 0 0 0 12 2m0 18a8 8 0 1 1 0-16a8 8 0 0 1 0 16m-1-5h2v2h-2zm0-8h2v6h-2z" />
                                                                </svg>
																Suspend
															</button>
														</form>
													...
													<c:otherwise>
														<form
															...
															<input type="hidden" name="adminId"
																value="${eachAdmin.adminId}"> <input
																type="hidden" name="actionDb" value="ACTIVE">
									...
									<tr>
										<td colspan="5"
											...
											<div
												...
												<p
													style="font-size: 14px; font-weight: 600; color: #475569; margin: 0;">No
													moderators found</p>
												<p style="font-size: 12px; color: #94A3B8; margin: 0;">No
													results match the selected filter.</p>
											</div>
										</td>
									</tr>
								</c:if>
		...
		<footer class="admin-dash-nav">
			<div class="footer-left">
				...
				<p class="footer-copy">©️ 2024 ClaimIt Digital Concierge. All
					rights reserved.</p>
			</div>
			<div class="footer-links">
				<a href="privacy-policy.html">Privacy Policy</a> <a
					href="term-of-service.html">Terms of Service</a> <a
					href="security-audit.html">Security Audit</a> <a
