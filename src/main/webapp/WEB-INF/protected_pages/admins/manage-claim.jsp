...
<link rel="stylesheet" href="${pageContext.request.contextPath}/css/manage-claim.css">
</head>
    ...
    <aside>
        <section class="admin-aside-outer">
            ...
            <div class="admin-aside-admin-function-outer">
                ...
                <div class="admin-aside-admin-function">
                    <a href="${pageContext.request.contextPath}/ManageClaim" class="admin-functions active-nav">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                            <rect width="24" height="24" fill="none" />
                ...
                <div class="admin-aside-admin-function">
                    <a href="${pageContext.request.contextPath}/ManageReport" class="admin-functions">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="16" viewBox="0 0 16 16">
                            ...
                            <path fill="#475569" d="M0 1.75C0 .784.784 0 1.75 0h12.5C15.216 0 16 .784 16 1.75v9.5A1.75 1.75 0 0 1 14.25 13H8.06l-2.573 2.573A1.458 1.458 0 0 1 3 14.543V13H1.75A1.75 1.75 0 0 1 0 11.25Zm1.75-.25a.25.25 0 0 0-.25.25v9.5c0 .138.112.25.25.25h2a.75.75 0 0 1 .75.75v2.19l2.72-2.72a.75.75 0 0 1 .53-.22h6.5a.25.25 0 0 0 .25-.25v-9.5a.25.25 0 0 0-.25-.25Zm7 2.25v2.5a.75.75 0 0 1-1.5 0v-2.5a.75.75 0 0 1 1.5 0M9 9a1 1 0 1 1-2 0a1 1 0 0 1 2 0" />
                        </svg>
        ...
        <main>
            <section class="admin-main-outer">
                ...
                <div class="mc-filter-bar">
                    <div class="mc-search">
                        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="2">
                            <circle cx="11" cy="11" r="8" />
                            <line x1="21" y1="21" x2="16.65" y2="16.65" />
                        </svg>
                        <input type="text" placeholder="Search by Claim ID or Item..."
                            form="mc-search-form" name="search"
                            value="${param.search}" />
                        <button type="submit" form="mc-search-form" class="mc-search-btn">Search</button>
                    </div>

                    <form id="mc-search-form" method="get" action="${pageContext.request.contextPath}/ManageClaim">
                    ...
                    <div class="mc-result-count">
                        Showing claims results
                    </div>
                </div>
                    ...
                    <table class="mc-table">
                        ...
                        <tbody class="mc-tr">
                            ...
                            <c:forEach var="eachClaim" items="${claims}">
                                ...
                                <c:choose>
                                    <c:when test="${dateParam == 'all'}">
                                        <c:set var="dateMatch" value="true" />
                                    </c:when>
                                    <c:otherwise>
                                        <c:set var="days" value="${dateParam == '7' ? 7 : dateParam == '90' ? 90 : 30}" />
                                        <c:set var="limitMs" value="${days * 24 * 60 * 60 * 1000}" />
                                        <c:set var="dateMatch" value="${eachClaim.createdAt == null || (now - eachClaim.createdAt.time) <= limitMs}" />
                                    </c:otherwise>
                                </c:choose>

                                <c:if test="${statusMatch && searchMatch && dateMatch}">
                                    <c:set var="visibleRows" value="${visibleRows + 1}" />
                                    <tr>
                                        <td><a href="#" class="mc-claim-id">#${eachClaim.claimId}</a></td>
                                        <td>
                                            <div class="mc-item-info">
                                                <div class="mc-item-thumb">
                                                    <c:choose>
                                                        <c:when test="${not empty eachClaim.proofImage}">
                                                            <img src="${pageContext.request.contextPath}/${fn:split(eachClaim.proofImage, ',')[0]}" alt="Proof Image" />
                                                        </c:when>
                                                        <c:otherwise>N/A</c:otherwise>
                                                    </c:choose>
                                                </div>
                                                <div>
                                                    <p class="mc-item-name">${eachClaim.itemTitle}</p>
                                                    <p class="mc-item-brand">${eachClaim.itemCategory}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <p class="mc-claimant-name">${eachClaim.claimedByName}</p>
                                            <p class="mc-claimant-email">${eachClaim.claimedByEmail}</p>
                                        </td>
                                        <td>
                                            <p class="mc-date">
                                                <fmt:formatDate value="${eachClaim.createdAt}" pattern="MMM dd, yyyy" />
                                            </p>
                                        </td>
                                        <td>
                                            <c:choose>
                                                <c:when test="${eachClaim.claimStatus == 'PENDING'}">
                                                    <span class="mc-badge pending">PENDING</span>
                                                </c:when>
                                                <c:when test="${eachClaim.claimStatus == 'APPROVED'}">
                                                    <span class="mc-badge approved">APPROVED</span>
                                                </c:when>
                                                <c:when test="${eachClaim.claimStatus == 'REJECTED'}">
                                                    <span class="mc-badge rejected">REJECTED</span>
                                                </c:when>
                                                <c:otherwise>
                                                    <span class="mc-badge review">IN REVIEW</span>
                                                </c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td>
                                            <div class="mc-actions">
                                                <form action="${pageContext.request.contextPath}/ViewClaim" method="GET" style="display:inline;">
                                                    <input type="hidden" name="claimId" value="${eachClaim.claimId}">
                                                    <button type="submit" class="mc-btn mc-btn-view">View</button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
                            ...
                            <c:if test="${visibleRows == 0}">
                                <tr>
                                    <td colspan="6" style="text-align: center; padding: 40px 16px; color: #94A3B8; font-size: 13px;">
                                        ...
                                        <c:choose>
                                            <c:when test="${not empty param.search}">
                                                <span>No claims match "<strong>${param.search}</strong>"</span>
                                            </c:when>
                                            <c:when test="${not empty param.status}">
                                                <span>No claims with status <strong>${param.status}</strong></span>
                                            </c:when>
                                        </c:choose>
                                    </td>
                                </tr>
                ...
                <div class="mc-stats-row">
                    <div class="mc-stat-blue">
                        ...
                        <c:set var="activeClaims" value="0" />
                        <c:forEach var="claim" items="${claims}">
                            <c:if test="${not empty claim.approvedBy}">
                                <c:set var="activeClaims" value="${activeClaims + 1}" />
                            </c:if>
                        </c:forEach>
                        <p class="mc-stat-blue-value">${activeClaims}</p>
                    ...
                    <div class="mc-stat-white">
                        ...
                        <div class="mc-progress-bar">
                            <div class="mc-progress-fill" style="width: ${approvedPercentage}%;"></div>
                        </div>
                    </div>
                </div>
