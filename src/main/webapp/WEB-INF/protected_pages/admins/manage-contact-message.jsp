...
<title>Contact</title>
<c:if test="${not empty suc || not empty err}">
	<meta http-equiv="refresh" content="3">
</c:if>
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/contact-message.css" />
</head>
	...
	<aside>
		<section class="admin-aside-outer">
			<div class="admin-aside-claimit-console">
				<p class="claimit-admin-console">
					ClaimIt Admin<br> Console
				</p>
		...
		<section class="rightside">
			...
			<section class="leftside">
				...
				<p>
					Review and manage inqueries from the Digital Concierge<br>
					portal.Maintain the editorial standard of response.
				</p>
			</section>
			...
			<section class="table-container">
				...
				<c:if test="${not empty suc}">
					<p
						style="background: #ECFDF5; color: #065F46; border: 1px solid #6EE7B7; border-radius: 8px; padding: 10px 16px; text-align: center; font-size: 14px; font-weight: 500; margin-bottom: 16px;">
						${suc}</p>
				</c:if>
				<c:if test="${not empty err}">
					<p
						style="background: #FEF2F2; color: #991B1B; border: 1px solid #FCA5A5; border-radius: 8px; padding: 10px 16px; text-align: center; font-size: 14px; font-weight: 500; margin-bottom: 16px;">
						${err}</p>
				</c:if>

				<div class="table-header">
					<span>ID</span> <span>SENDER</span> <span>MESSAGE PREVIEW</span> <span>DATE</span>
					<span>STATUS</span> <span>ACTION</span>
				</div>

				<!-- variables declarations for msg counts  -->
				<c:set var="readCount" value="0" />
				<c:set var="unReadCount" value="0" />

				<c:forEach var="each" items="${contactMessage}">
					<c:choose>
						<c:when test="${!each.isRead}">
							<c:set var="unReadCount" value="${unReadCount + 1}" />
							<div class="row">
								<div class="MSG">MSG-${each.contactMessageId}</div>

								<div class="sender">
									<div class="avatar-un">${fn:toUpperCase(fn:substring(each.email, 0, 2))}</div>
									<div class="sender-info">
										<c:choose>
											<c:when test="${each.userId == 0}">N/A</c:when>
											<c:otherwise>${each.userId}</c:otherwise>
										</c:choose>
								...
								<div>
									<div class="message-title">
										<c:choose>
											<c:when test="${fn:length(each.message) < 7}">N/A</c:when>
											<c:otherwise>${fn:substring(each.message, 0, 7)}</c:otherwise>
										</c:choose>
									</div>
									<div class="message-desc">${each.message}</div>
								</div>

								<div class="date">
									<fmt:formatDate value="${each.submittedAt}"
										pattern="MMM dd, yyyy" />
									<span><fmt:formatDate value="${each.submittedAt}"
								...
								<div>
									<form
										action="${pageContext.request.contextPath}/ManageContactMessage"
										method="POST">
										<input type="hidden" name="messageId"
											value="${each.contactMessageId}">
						...
						<c:otherwise>
							...
							<div class="row">
								...
								<div class="sender">
									<div class="avatar">${fn:toUpperCase(fn:substring(each.email, 0, 2))}</div>
									<div class="sender-info">
										<c:choose>
											<c:when test="${each.userId == 0}">N/A</c:when>
											<c:otherwise>${each.userId}</c:otherwise>
			...
			<section class="Message-bottom">
				<div class="left-side">
					<div class="icon blue">
						<img src="${pageContext.request.contextPath}/images/mail.jpg"
							alt="Email logo">
					</div>
					<div class="text">
						<p>UNREAD MESSAGE</p>
				...
				<div class="right-side">
					<div class="icon green">
						<img src="${pageContext.request.contextPath}/images/tick.jpg"
							alt="check logo">
					</div>
					<div>
						<div class="text">
