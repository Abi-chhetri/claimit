...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page
	import="java.util.List, com.claimit.model.Claim, java.text.SimpleDateFormat , com.claimit.enums.ClaimStatus"%>
<%
...
String selectedDate = request.getParameter("date") != null ? request.getParameter("date") : "30";
%>
<!DOCTYPE html>
<html lang="en">
...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/manage-claim.css">
</head>
			...
			<section class="admin-main-outer">
				...
				<div class="mc-header">
					<div>
						...
						<p class="mc-subtitle">Review and process ownership claims for
							recovered items.</p>
					</div>
					<div class="mc-avatars">
						<a href="" class="admin-dashboard-admin-logo"> <svg
				...
				<div class="mc-filter-bar">
					...
					<form id="mc-search-form" method="get"
						action="${pageContext.request.contextPath}/ManageClaim">
						...
						<form method="get"
							...
							<select name="status" onchange="this.form.submit()">
								...
								<option value="IN_REVIEW"
									<%="IN_REVIEW".equals(request.getParameter("status")) ? "selected" : ""%>>In
									Review</option>
								<option value="APPROVED"
									<%="APPROVED".equals(request.getParameter("status")) ? "selected" : ""%>>Approved</option>
					...
					<div class="mc-filter-pill">
						<form method="get"
							...
							<select name="date" onchange="this.form.submit()">
								<option value="30"
									<%=request.getParameter("date") == null || "30".equals(request.getParameter("date")) ? "selected" : ""%>>Last
									30 Days</option>
								<option value="7"
									<%="7".equals(request.getParameter("date")) ? "selected" : ""%>>Last
									7 Days</option>
								<option value="90"
									<%="90".equals(request.getParameter("date")) ? "selected" : ""%>>Last
									90 Days</option>
								<option value="all"
									<%="all".equals(request.getParameter("date")) ? "selected" : ""%>>All
									Time</option>
							</select>
						</form>
					</div>
					...
					<table class="mc-table">
						...
						<tbody class="mc-tr">
							...
							for (Claim eachClaim : claims) {
								...
								// status filter
								if (!selectedStatus.isEmpty() && !eachClaim.getClaimStatus().equals(selectedStatus))
									continue;

								// search filter
								if (!searchQuery.isEmpty() && !String.valueOf(eachClaim.getClaimId()).contains(searchQuery)
								&& !eachClaim.getItemTitle().toLowerCase().contains(searchQuery))
									continue;

								// date filter
								if (!selectedDate.equals("all") && eachClaim.getCreatedAt() != null) {
									int days = selectedDate.equals("7") ? 7 : selectedDate.equals("90") ? 90 : 30;
									long diff = now - eachClaim.getCreatedAt().getTime();
									if (diff > (long) days * 24 * 60 * 60 * 1000) // days * 24 hrs * 60 min * 60 sec * 1000 mili converting day into ms
								continue;
								}
								...
								<td>
									<div class="mc-item-info">
										<div class="mc-item-thumb">
											<%
											if (eachClaim.getProofImage() != null) {
											%>
											<img
												src="${pageContext.request.contextPath}/<%=eachClaim.getProofImage().split(",")[0]%>"
												alt="Proof Image" />
											<%
											} else {
											%>N/A<%
								...
								<td>
									<div class="mc-actions">
										<form action="${pageContext.request.contextPath}/ViewClaim" method="GET" style="display:inline;">
										    <input type="hidden" name="claimId" value="<%= eachClaim.getClaimId() %>">
							...
							<tr>
								<td colspan="6"
									style="text-align: center; padding: 40px 16px; color: #94A3B8; font-size: 13px;">
									...
									<p>No results found</p> <%
									 ...
									 %> <span>No claims match "<strong><%=searchQuery%></strong>"</span> 
									 <%
									 } else if (!selectedStatus.isEmpty()) {
									 %> <span>No claims with status <strong><%=selectedStatus%></strong> </span> <%
									 }
									 %>
								</td>
							</tr>
				...
				<div class="mc-stats-row">
					...
					<div class="mc-stat-white">
						...
						<p class="mc-progress-label">${approvedPercentage}%claims
							resolved out of ${approvedCount}</p>
