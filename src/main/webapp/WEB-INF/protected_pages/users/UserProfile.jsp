...
<c:if test="${not empty msg}">
	<meta http-equiv="refresh"
		content="3;url=${pageContext.request.contextPath}/UserProfile">
</c:if>
</head>
...
<body>
	...
	<main class="main-content">
		...
		<section class="profile-hero" aria-labelledby="profile-heading">
			...
			<p class="profile-subtitle">Manage your personal information and
				account security.</p>
		</section>
		...
		<section class="profile-grid" aria-label="Profile sections">
			...
			<article class="card personal-info"
				aria-labelledby="personal-info-heading">

				<header class="card-header">
					<h2 id="personal-info-heading" class="card-title">Personal
						Information</h2>
				</header>
				...
				<div class="info-field">
					...
					<div class="field-row">
						...
						<span class="field-label">Full Name</span> <a
							href="${pageContext.request.contextPath}/UserProfile?action=${param.action eq 'editName' ? 'view' : 'editName'}"
							class="edit-btn"> ${param.action eq 'editName' ? '✕ Cancel' : '✎ Edit'}

						</a>

					</div>

					<!-- Show edit form for full name -->
					<c:choose>

						<c:when test="${param.action eq 'editName'}">
				...
				<div class="info-field">
					...
					<div class="field-row">
						...
						<span class="field-label">Email Address</span> <span
							class="field-locked"> <svg
								xmlns="http://www.w3.org/2000/svg" width="13" height="13"
								viewBox="0 0 24 24">

                                <path fill="#94A3B8"
									d="M18 8h-1V6A5 5 0 0 0 7 6v2H6a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V10a2 2 0 0 0-2-2m-6 9a2 2 0 1 1 0-4a2 2 0 0 1 0 4m3.1-9H8.9V6a3.1 3.1 0 0 1 6.2 0z" />
				...
				<div class="info-field">
					...
					<div class="field-row">
						...
						<span class="field-label">Phone Number</span> <a
							href="${pageContext.request.contextPath}/UserProfile?action=${param.action eq 'editPhone' ? 'view' : 'editPhone'}"
							class="edit-btn"> ${param.action eq 'editPhone' ? '✕ Cancel' : '✎ Edit'}

						</a>

					</div>

					<!-- Show edit form for phone -->
					<c:choose>

						<c:when test="${param.action eq 'editPhone'}">
				...
				<div class="identity-verified" role="status">
					...
					<div class="verified-text">
						...
						<p>Your identity has been verified. This helps build trust
							within the ClaimIt community.</p>
			...
			<aside class="right-column">
				...
				<article class="card concierge-card"
					aria-labelledby="concierge-heading">

					<img
						src="${not empty user.profilePhoto ? pageContext.request.contextPath.concat('/').concat(user.profilePhoto) : pageContext.request.contextPath.concat('/images/BG.png')}"
						...
						<p class="concierge-sub">
							...
							<c:choose>
								...
								<c:when test="${not empty user.approvedAt}">
									<fmt:formatDate value="${user.approvedAt}" pattern="MMM yyyy" />
								</c:when>
				...
				<article class="card preferences-card">
					...
					<form action="${pageContext.request.contextPath}/UserProfile"
						...
						<c:choose>
							<c:when test="${param.action eq 'uploadPic'}">
								<div class="upload-controls">
									...
									<a href="${pageContext.request.contextPath}/UserProfile"
										class="cancel-upload-btn"> Cancel </a>
								</div>
							</c:when>
							<c:otherwise>
								<a
									href="${pageContext.request.contextPath}/UserProfile?action=uploadPic"
									class="update-pic-btn"> <svg
		...
		<section class="change-password" aria-labelledby="password-heading">
			<header class="password-header">
				<h2 id="password-heading" class="password-title">Change
					Password</h2>
				<p class="password-subtitle">Update your password to keep your
					account secure. Use at least 8 characters.</p>
			</header>
			<form action="${pageContext.request.contextPath}/UserProfile"
				method="post">
				...
				<div class="password-fields">
					<div class="field-group full-width">
						<label class="field-label" for="current-password"> Current
							Password </label> <input class="password-input" type="password"
							id="current-password" name="current_password"
							placeholder="••••••••" />
					</div>
					<div class="password-row">
						<div class="field-group">
							<label class="field-label" for="new-password"> New
								Password </label> <input class="password-input" type="password"
								id="new-password" name="new_password" placeholder="••••••••" />
						</div>
						<div class="field-group">
							<label class="field-label" for="confirm-password">
								Confirm New Password </label> <input class="password-input"
								type="password" id="confirm-password" name="confirm_password"
								placeholder="••••••••" />
						</div>
					</div>
					<button type="submit" class="update-password-btn">Update
						Password</button>
				</div>
			</form>
		</section>
