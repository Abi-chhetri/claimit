				...
				<div class="admin-aside-admin-function">
					<a href="${pageContext.request.contextPath}/AdminDashBoard"
						class="admin-functions"> <svg
							...
							<path fill="#475569"
								d="M13 9V3h8v6zM3 13V3h8v10zm10 8V11h8v10zM3 21v-6h8v6zm2-10h4V5H5zm10 8h4v-6h-4zm0-12h4V5h-4zM5 19h4v-2H5zm4-2" /></svg>
				...
				<div class="admin-aside-admin-function">
					<a href="${pageContext.request.contextPath}/ManageUser"
						class="admin-functions"> <svg
							...
							<path fill="#475569"
								d="M12.52 2.678A3 3 0 0 1 14.5 5.5v1c0 1.297-.848 2.581-2 3q1.01-1.379 1.01-3.5c0-2.122-.331-2.523-.99-3.322M17.5 17.5h1a1 1 0 0 0 1-1v-.728c0-2.17-1.71-3.83-3.847-4.667c0 0 2.847 2.395 1.847 6.395" /></g></svg>
				...
				<div class="admin-aside-admin-function">
					<a href="${pageContext.request.contextPath}/ManageModerators"
						class="admin-functions"> <svg
							...
							<path fill="#475569"
								d="M4 4v3q2 0 3-3M4 4V1L1 2v2m3-4l4 2c0 8-8 8-8 0" /></svg>
				...
				<div class="admin-aside-admin-function">
					<a href="${pageContext.request.contextPath}/ContactMessage"
						class="admin-functions"> <svg
							...
							<rect width="32" height="32" fill="none" />
							<path fill="#475569"
								d="M2.004 9.303A4.5 4.5 0 0 1 6.5 5h19a4.5 4.5 0 0 1 4.496 4.303l-1.476.82L16 16.864L3.48 10.123zM2 11.588V22.5A4.5 4.5 0 0 0 6.5 27h19a4.5 4.5 0 0 0 4.5-4.5V11.588l-.526.293l-13 7a1 1 0 0 1-.948 0L2.514 11.874z" /></svg>
				...
				<div class="admin-aside-admin-function">
					<a href="${pageContext.request.contextPath}/AdminLogs"
						class="admin-functions"> <svg
							...
							<rect width="24" height="24" fill="none" />
							<path fill="#475569"
								d="M12 2c5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12h2a8 8 0 1 0 1.385-4.5H8v2H2v-6h2V6a9.99 9.99 0 0 1 8-4m1 5v4.585l3.243 3.243l-1.415 1.415L11 12.413V7z" /></svg>
				...
				<div id="admin-logo">
					<a href="${pageContext.request.contextPath}/Logout"
						class="admin-logo"> <svg xmlns="http://www.w3.org/2000/svg"
							...
							<path
								d="M16.53 7.97a.75.75 0 0 0-1.06 0v3.276H9.5a.75.75 0 0 0 0 1.5h5.97v3.284a.75.75 0 0 0 1.06 0l3.5-3.5a.75.75 0 0 0 .22-.532v-.002a.75.75 0 0 0-.269-.575z" /></g></svg>
						<p>Logout</p>
					</a>
				</div>
				<div id="admin-logo">
			...
			<section class="admin-main-outer">
				...
				<div class="vc-header">
					<div class="vc-header-left">
						<a href="${pageContext.request.contextPath}/ManageClaim"
							class="vc-back-btn"> <svg xmlns="http://www.w3.org/2000/svg"
						...
						<div>
							...
							<p class="vc-subtitle">
								Submitted on
								<fmt:formatDate value="${claim.createdAt}"
					...
					<div class="vc-header-right">
						<c:choose>
							<c:when test="${claim.claimStatus == 'PENDING'}">
								<span class="vc-badge vc-badge-pending">PENDING</span>
							</c:when>
							<c:when test="${claim.claimStatus == 'IN_REVIEW'}">
								<span class="vc-badge vc-badge-review">IN REVIEW</span>
							</c:when>
							<c:when test="${claim.claimStatus == 'APPROVED'}">
								<span class="vc-badge vc-badge-approved">APPROVED</span>
							</c:when>
							<c:when test="${claim.claimStatus == 'REJECTED'}">
				...
				<div class="vc-body">
					...
					<div class="vc-left">
						...
						<div class="vc-card">
							...
							<div class="vc-info-row">
								<span class="vc-info-label">Full Name</span> <span
									class="vc-info-value">${user.fullName}</span>
							</div>
							<div class="vc-info-row">
								<span class="vc-info-label">Email</span> <span
									class="vc-info-value">${user.email}</span>
							</div>
							<div class="vc-info-row">
								<span class="vc-info-label">User ID</span> <span
									class="vc-info-value">#${user.userId}</span>
							</div>
						</div>
						...
						<div class="vc-card">
							...
							<div class="vc-info-row">
								<span class="vc-info-label">Item Title</span> <span
									class="vc-info-value">${item.title}</span>
							</div>
							<div class="vc-info-row">
								<span class="vc-info-label">Category</span> <span
									class="vc-info-value">${item.category}</span>
							</div>
							<div class="vc-info-row">
								<span class="vc-info-label">Item ID</span> <span
									class="vc-info-value">#${item.itemId}</span>
							</div>
						</div>
						...
						<div class="vc-card">
							...
							<p class="vc-description">
								<c:choose>
									<c:when test="${not empty claim.ownershipDescription}">${claim.ownershipDescription}</c:when>
						...
						<div class="vc-card">
							...
							<p class="vc-description">
								<c:choose>
									<c:when test="${not empty claim.adminNotes}">${claim.adminNotes}</c:when>
									<c:otherwise>No notes added yet.</c:otherwise>
								</c:choose>
							</p>
						</div>
					...
					<div class="vc-right">
						...
						<div class="vc-card">
							...
							<c:choose>
								<c:when test="${not empty claim.proofImage}">
									<c:forTokens items="${claim.proofImage}" delims="," var="img">
										<div class="vc-proof-img-wrap">
											<img src="${pageContext.request.contextPath}/${img}"
												alt="Proof Image" class="vc-proof-img" />
										</div>
									</c:forTokens>
								</c:when>
								<c:otherwise>
							...
							<div class="vc-timeline">
								<div class="vc-timeline-item">
									...
									<div>
										...
										<p class="vc-timeline-date">
											<fmt:formatDate value="${claim.createdAt}"
												pattern="MMM dd, yyyy hh:mm a" />
										</p>
									</div>
								</div>
								<c:if test="${not empty claim.approvedAt}">
									<div class="vc-timeline-item">
										<div class="vc-timeline-dot vc-dot-green"></div>
										<div>
											<p class="vc-timeline-label">
												<c:choose>
													<c:when test="${claim.claimStatus == 'APPROVED'}">Approved</c:when>
													<c:otherwise>Reviewed</c:otherwise>
												</c:choose>
												<c:if test="${not empty claim.approvedBy}"> by Admin #${claim.approvedBy}</c:if>
											</p>
											<p class="vc-timeline-date">
												<fmt:formatDate value="${claim.approvedAt}"
													pattern="MMM dd, yyyy hh:mm a" />
											</p>
										</div>
									</div>
								...
								<c:if
									test="${claim.claimStatus == 'REJECTED' && not empty claim.approvedAt}">
									<div class="vc-timeline-item">
						...
						<div class="vc-card">
							...
							<div class="vc-actions">
								...
								<c:choose>
									<c:when test="${showNotesForm == true}">
										<!-- Notes form — shown after first click -->
										<p class="vc-notes-prompt">
											You are about to mark this claim as <strong>${func:toUpperCase(pendingAction)}</strong>.
											Add a note before confirming.
										</p>
										<form method="POST"
											action="${pageContext.request.contextPath}/ViewClaim">
											...
											<div class="vc-notes-actions">
												<button type="submit" class="vc-btn vc-btn-${pendingAction}">
													Confirm ${func:substring(pendingAction, 0, 1).toUpperCase()}${func:substring(pendingAction, 1, func:length(pendingAction))}
												</button>
												<a
													href="${pageContext.request.contextPath}/ViewClaim?claimId=${claim.claimId}"
									...
									<c:when test="${claim.claimStatus == 'PENDING'}">
										<form method="POST"
											...
											<button type="submit" class="vc-btn vc-btn-approve">Approve
												Claim</button>
										</form>
										<form method="POST"
											action="${pageContext.request.contextPath}/ViewClaim">
											...
											<button type="submit" class="vc-btn vc-btn-reject">Reject
												Claim</button>
										</form>
										<form method="POST"
											action="${pageContext.request.contextPath}/ViewClaim">
											...
											<button type="submit" class="vc-btn vc-btn-review">Mark
												In Review</button>
										</form>
									</c:when>
									...
									<c:otherwise>
										<c:choose>
											<c:when test="${claim.claimStatus == 'APPROVED'}">
												<p style="font-size: 14px; color: green;">The claim has
													already been approved.</p>
											</c:when>
											<c:when test="${claim.claimStatus == 'REJECTED'}">
												<p style="font-size: 14px; color: red;">The claim has
													already been rejected.</p>
											</c:when>
											<c:otherwise>
												<p style="font-size: 14px; color: orange;">The claim has
													already been ${func:toLowerCase(claim.claimStatus)}.</p>
											</c:otherwise>
										</c:choose>
									</c:otherwise>
								</c:choose>
