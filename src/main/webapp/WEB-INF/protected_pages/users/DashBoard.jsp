<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%@ taglib prefix="c"   uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Claim it: User DashBoard</title>
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/user_dashboard.css">
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/header-footer.css">

    <%-- Auto-redirect to Dashboard after 3s if flash message exists --%>
    <c:if test="${not empty sessionScope.flashMessage}">
        <meta http-equiv="refresh" content="3;url=${pageContext.request.contextPath}/DashBoard">
    </c:if>
</head>

<body>

    <%-- Show flash message once then remove it from session --%>
    <c:if test="${not empty sessionScope.flashMessage}">
        <div class="msg-box">
            ${sessionScope.flashMessage}
            <c:remove var="flashMessage" scope="session" />
        </div>
    </c:if>

    <header class="header">
        <div class="header1">
            <span class="claimit-onheader">ClaimIt</span>
            <nav class="navbar">
                <a href="${pageContext.request.contextPath}/Home">Home</a>
                <a href="${pageContext.request.contextPath}/Browse">Browse</a>
                <a href="${pageContext.request.contextPath}/DashBoard" class="active">Dashboard</a>
                <a href="${pageContext.request.contextPath}/ReportFoundItem">Report Found Item</a>
                <a href="${pageContext.request.contextPath}/MyClaim">Claims</a>
                <a href="${pageContext.request.contextPath}/About">About</a>
            </nav>
        </div>
        <div class="header2">
            <form action="${pageContext.request.contextPath}/Notification" method="get">
                <button type="submit" class="noti-acc-button">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                        <path fill="currentColor"
                            d="M19.29 17.29L18 16v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-1.29 1.29c-.63.63-.19 1.71.7 1.71h13.17c.9 0 1.34-1.08.71-1.71M16 17H8v-6c0-2.48 1.51-4.5 4-4.5s4 2.02 4 4.5zm-4 5c1.1 0 2-.9 2-2h-4a2 2 0 0 0 2 2" />
                    </svg>
                </button>
            </form>
            <form action="${pageContext.request.contextPath}/UserProfile" method="get">
                <button class="noti-acc-button" type="submit">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                        <path fill="currentColor"
                            d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10s10-4.48 10-10S17.52 2 12 2m0 4c1.93 0 3.5 1.57 3.5 3.5S13.93 13 12 13s-3.5-1.57-3.5-3.5S10.07 6 12 6m0 14c-2.03 0-4.43-.82-6.14-2.88a9.95 9.95 0 0 1 12.28 0C16.43 19.18 14.03 20 12 20" />
                    </svg>
                </button>
            </form>
            <form action="${pageContext.request.contextPath}/Logout" method="get">
                <button type="submit" class="logout-button">Logout</button>
            </form>
        </div>
    </header>

    <main>
        <article class="main-page">

            <div class="wel-nor-message">
                <p class="wel">Welcome back, ${requestScope.user.fullName}</p>
                <p class="nor">Your concierge for lost and found belongings.</p>
            </div>

            <div class="container-re-pe-fo-box">

                <div class="report-box">
                    <a href="" class="report-box-txt-num">
                        <span class="report-box-txt">TOTAL REPORTED</span><br>
                        <span class="report-box-number">${requestScope.userReportCount}</span>
                    </a>
                    <svg xmlns="http://www.w3.org/2000/svg" width="56" height="40" viewBox="0 0 32 32">
                        <path fill="#007bff" d="M20 21h-8a2 2 0 0 1-2-2v-2a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v2a2 2 0 0 1-2 2m-8-4v2h8v-2Z" />
                        <path fill="#007bff" d="M28 4H4a2 2 0 0 0-2 2v4a2 2 0 0 0 2 2v16a2 2 0 0 0 2 2h20a2 2 0 0 0 2-2V12a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2m-2 24H6V12h20Zm2-18H4V6h24z" />
                    </svg>
                </div>

                <div class="pending-box">
                    <a href="" class="report-box-txt-num">
                        <span class="pending-box-txt">PENDING CLAIMS</span><br>
                        <span class="pending-box-number">${requestScope.pending}</span>
                    </a>
                    <svg xmlns="http://www.w3.org/2000/svg" width="56" height="40" viewBox="0 0 24 24">
                        <path fill="#ffa629" d="M13.463 20.538Q12 19.075 12 17t1.463-3.537T17 12t3.538 1.463T22 17t-1.463 3.538T17 22t-3.537-1.463m5.212-1.162l.7-.7L17.5 16.8V14h-1v3.2zM5 21q-.825 0-1.412-.587T3 19V5q0-.825.588-1.412T5 3h4.175q.275-.875 1.075-1.437T12 1q1 0 1.788.563T14.85 3H19q.825 0 1.413.588T21 5v6.25q-.45-.325-.95-.55T19 10.3V5h-2v3H7V5H5v14h5.3q.175.55.4 1.05t.55.95zm7.713-16.288Q13 4.425 13 4t-.288-.712T12 3t-.712.288T11 4t.288.713T12 5t.713-.288" />
                    </svg>
                </div>

                <div class="found-box">
                    <a href="" class="report-box-txt-num">
                        <span class="found-box-txt">ACCEPTED CLAIMS</span><br>
                        <span class="found-box-number">${requestScope.approved}</span>
                    </a>
                    <svg xmlns="http://www.w3.org/2000/svg" width="56" height="40" viewBox="0 0 24 24">
                        <g fill="none" stroke="#14ae5c" stroke-linejoin="round" stroke-width="1.5">
                            <path stroke-linecap="round" d="M11 20h-.5c-3.759 0-5.638 0-6.893-.99a4.4 4.4 0 0 1-.554-.523C2 17.307 2 15.537 2 12s0-5.306 1.053-6.487q.253-.284.554-.522C4.862 4 6.741 4 10.5 4h3c3.759 0 5.638 0 6.892.99q.302.24.555.523C21.896 6.577 21.99 8.118 22 11" />
                            <path d="M2 9h20" />
                            <path stroke-linecap="round" d="M14 18s1 0 2 2c0 0 3.177-5 6-6" />
                        </g>
                    </svg>
                </div>
            </div>

            <section class="quick-logo-txt">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 16 16">
                    <path fill="#007bff" d="M11.251.068a.5.5 0 0 1 .227.58L9.677 6.5H13a.5.5 0 0 1 .364.843l-8 8.5a.5.5 0 0 1-.842-.49L6.323 9.5H3a.5.5 0 0 1-.364-.843l8-8.5a.5.5 0 0 1 .615-.09zM4.157 8.5H7a.5.5 0 0 1 .478.647L6.11 13.59l5.732-6.09H9a.5.5 0 0 1-.478-.647L9.89 2.41z" />
                </svg>
                <p class="quick-action">QUICK ACTION</p>
            </section>

            <section class="report-lost-found-needhelp">
                <div class="report-lost-item">
                    <a href="${pageContext.request.contextPath}/MyClaim">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                            <path fill="#fff" d="M19 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2m-7 3a3 3 0 1 1 0 6a3 3 0 0 1 0-6m5 12H7v-.5c0-2.33 3.33-3.5 5-3.5s5 1.17 5 3.5z" />
                        </svg>
                        My Claims
                    </a>
                </div>

                <div class="report-found-item">
                    <a href="${pageContext.request.contextPath}/ReportFoundItem">
                        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 50 50">
                            <path fill="#14ae5c" d="M25 42c-9.4 0-17-7.6-17-17S15.6 8 25 8s17 7.6 17 17s-7.6 17-17 17m0-32c-8.3 0-15 6.7-15 15s6.7 15 15 15s15-6.7 15-15s-6.7-15-15-15" />
                            <path fill="#14ae5c" d="M16 24h18v2H16z" />
                            <path fill="#14ae5c" d="M24 16h2v18h-2z" />
                        </svg>
                        Report Found Item
                    </a>
                </div>

                <div class="browse-item">
                    <a href="${pageContext.request.contextPath}/Browse">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 100 100">
                            <path fill="#0d99ff" d="M45 0a45 45 0 0 0-6.85.52a45 45 0 0 0-6.521 1.505C16.836 6.63 5.305 18.666 1.418 33.76v.004A45 45 0 0 0 .52 38.15v.004q-.171 1.117-.288 2.248v.002a45 45 0 0 0-.173 6.908v.002a45 45 0 0 0 .173 2.282v.002q.116 1.132.288 2.248v.004a45 45 0 0 0 .898 4.386v.004c3.887 15.094 15.418 27.13 30.21 31.735q1.056.328 2.132.607h.004q1.075.277 2.172.502h.003q1.097.226 2.211.396h.004q1.117.171 2.248.288h.002a45 45 0 0 0 6.907.173c1.158-.058 2.3-.175 3.433-.32c.364-.046.73-.083 1.092-.139l.01-.002a45 45 0 0 0 6.525-1.505a44.9 44.9 0 0 0 15.277-8.301c-4.049 1.638-8.465 2.323-12.822 2.05A39.9 39.9 0 0 1 45 85a40 40 0 0 1-14.744-2.809c.329-.314.698-.574 1.166-.709c5.833 1.41 3.295-5.805 7.256-7.437c2.653-4.864-5.818-5.64-8.842-7.41c-5.114-.773-4.635-6.996-9.28-8.342c-3.803-.607-2.422-6.088-5.21-5.795c-2.407.447-3.362-2.863-4.703-2.627c-.324.342-1.68-2.222-3.137.604c-3.487-.772-1.964-4.278-2.469-6.942A40 40 0 0 1 7.56 30.891c.852.77.446 3.23.587 4.898c1.66.711.88-6.277 4.329-6.44c2.346-2.641 5.068-4.89 7.92-6.708c1.113.793 3.505.697 5.382-1.065c-2.091-.42-2.24-2.017.555-2.644c4.448-1.912-2.006 2.095 1.625 2.673c3.942 1.162.598-3.428 2.467-5.222c-1.956-1.332 1.183-6.512-2.238-4.211c1.183-.846-.657-2.433.4-3.053c.437 1.224 2.636 2.905 3.418.082c1.55.265 1.857-1.244 2.316-2.754a40 40 0 0 1 3.399-.777c-.384 1.447-1.774 3.682-1.887 5.13c1.794 5.23 4.822-2.016 8.012-2.195c2.755.094 3.317-1.75 3.396-3.539c3.688.203 7.239.91 10.592 2.041c.286 2.569 1.394 6.622 4.205 5.727c-.586.13 1.866 2.554.598 2.96c-.674 4.536-4.353-3.024-6.219-2.925c-2.789.47.53 1.85-.477 2.526c-2.64 2.267 2.285 4.646 1.887.835c.323 3.073 1.365 2.977 3.856 2.817c-4.918.83 1.02 2.59.867 4.242a30 30 0 0 1 4.806.373c.367-.276.834-.412 1.457-.336a2.4 2.4 0 0 0-.085.57q.553.115 1.101.25c.128-.163.705.023 1.33.362l.026.008c1.05.303 2.076.677 3.08 1.103c.15-.429.422-.582 1.01-.035c.19.33.385.578.58.768c.307.154.608.32.91.486c.91.136 1.7-.658 1.95-.717c.468-.13 1.054-.092 1.57.006q.719 1.352 1.337 2.762c-.326.134-.722.247-1.129.363c4.12 3.154 7.412 7.396 9.365 12.207a44.7 44.7 0 0 0-2.615-11.98c-.092-.251-.196-.496-.293-.745c-.167-.43-.332-.861-.511-1.285c-.159-.372-.333-.736-.5-1.103c-.133-.29-.257-.583-.395-.87c-.193-.399-.401-.789-.606-1.181c-.132-.254-.255-.513-.392-.764a45 45 0 0 0-1.066-1.851c-6.016-9.9-15.741-17.3-27.243-20.262l-.013-.004A45 45 0 0 0 51.846.52l-.008-.002a45 45 0 0 0-2.242-.286h-.006A45 45 0 0 0 47.312.06h-.001A46 46 0 0 0 45 0" color="#0d99ff" />
                            <path fill="#0d99ff" d="M66.129 27.495c-6.422-.87-13.175.702-18.72 4.925c-11.09 8.444-13.247 24.366-4.802 35.456c7.905 10.38 22.34 12.883 33.25 6.237l2.083 2.736a2.69 4.051 52.712 0 0 .106 2.494l15.12 19.855a2.69 4.051 52.712 0 0 4.852-.314a2.69 4.051 52.712 0 0 1.594-4.595l-15.12-19.855a2.69 4.051 52.712 0 0-2.376-.765l-2.084-2.737c9.31-8.75 10.737-23.33 2.833-33.71c-4.223-5.546-10.314-8.857-16.736-9.727m-.75 5.537a19.62 19.62 0 0 1 13.013 7.596a19.635 19.635 0 0 1-3.735 27.577a19.635 19.635 0 0 1-27.576-3.735a19.635 19.635 0 0 1 3.734-27.576a19.6 19.6 0 0 1 14.564-3.862" color="#0d99ff" />
                        </svg>
                        Browse Item
                    </a>
                </div>

                <div class="need-help">
                    <p class="need-help-txt">Need Help?</p>
                    <p class="need-help-nor-txt">Our concierge team is here to help you reunite with your lost items 24/7. Connect with a specialist now.</p>
                    <a href="${pageContext.request.contextPath}/About#contact" class="need-help-btn-a-link">
                        <button class="need-help-btn">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                <path fill="#0d99ff" d="M18.72 14.76c.35-.85.54-1.76.54-2.76c0-.72-.11-1.41-.3-2.05c-.65.15-1.33.23-2.04.23A9.07 9.07 0 0 1 9.5 6.34a9.2 9.2 0 0 1-4.73 4.88c-.04.25-.04.52-.04.78A7.27 7.27 0 0 0 12 19.27c1.05 0 2.06-.23 2.97-.64c.57 1.09.83 1.63.81 1.63c-1.64.55-2.91.82-3.78.82c-2.42 0-4.73-.95-6.43-2.66a9 9 0 0 1-2.24-3.69H2v-4.55h1.09a9.09 9.09 0 0 1 15.33-4.6a9 9 0 0 1 2.47 4.6H22v4.55h-.06L18.38 18l-5.3-.6v-1.67h4.83zm-9.45-2.99c.3 0 .59.12.8.34a1.136 1.136 0 0 1 0 1.6c-.21.21-.5.33-.8.33c-.63 0-1.14-.5-1.14-1.13s.51-1.14 1.14-1.14m5.45 0c.63 0 1.13.51 1.13 1.14s-.5 1.13-1.13 1.13s-1.14-.5-1.14-1.13a1.14 1.14 0 0 1 1.14-1.14" />
                            </svg>
                            <p class="need-help-btn-txt">Contact Concierge</p>
                        </button>
                    </a>
                </div>
            </section>

            <section class="recent-activity">
                <div class="activity-container">

                    <div class="activity-one">
                        <p class="activity-one-txt">Recent Activity</p>
                        <a href="#" class="activity-one-link">Log Status</a>
                    </div>

                    <%-- Loop over notifications; show message, formatted time, and type for each --%>
                    <c:forEach var="eachNot" items="${requestScope.notif}">
                        <div class="activity-box">
                            <div class="activity-main-txt">
                                <p class="activity-txt">${eachNot.message}</p>
                                <%-- Format createdAt timestamp to HH:mm a pattern --%>
                                <p class="activity-status-txt">Time :
                                    <fmt:formatDate value="${eachNot.createdAt}" pattern="HH:mm a" />
                                </p>
                            </div>
                            <div class="status-sign" id="returned-status">${eachNot.type}</div>
                        </div>
                    </c:forEach>

                </div>
            </section>

        </article>
    </main>

    <footer class="site-footer">
        <div class="footer-left">
            <span class="footer-brand">ClaimIt</span>
        </div>
        <div class="footer-middle">
            <a href="#">Privacy Policy</a>
            <a href="#">Terms of Service</a>
            <a href="#">Help Center</a>
            <a href="#">Contact Us</a>
        </div>
        <div class="footer-right">
            <p>&copy; 2026 ClaimIt Protocol. All rights reserved.</p>
        </div>
    </footer>

</body>
</html>